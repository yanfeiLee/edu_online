...
import com.lyf.qz.beans.{DwdQzPaperView, DwdQzPoint, DwdQzQuestion}
...
/**
  * Project: edu_online
  * Create by lyf3312 on 20/04/02 14:12
  * Version: 1.0
  */
object EtlOdsData2DwdService {
  ...
  def etlQzChapter(ssc: SparkContext, sparkSession: SparkSession) = {
    ...
    ssc.textFile("/user/lee/ods/QzChapter.log").filter(item => {
  ...
  def etlQzChapterList(ssc: SparkContext, sparkSession: SparkSession) = {
    ...
    ssc.textFile("/user/lee/ods/QzChapterList.log").filter(item => {
      val obj = ParseJsonData.getJsonObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzPoint(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/lee/ods/QzPoint.log").filter(item => {
      val obj = ParseJsonData.getJsonObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzPointQuestion(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/lee/ods/QzPointQuestion.log").filter(item => {
      val obj = ParseJsonData.getJsonObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzSiteCourse(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/lee/ods/QzSiteCourse.log").filter(item => {
      val obj = ParseJsonData.getJsonObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzCourse(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/lee/ods/QzCourse.log").filter(item => {
      val obj = ParseJsonData.getJsonObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzCourseEdusubject(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/lee/ods/QzCourseEduSubject.log").filter(item => {
      val obj = ParseJsonData.getJsonObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzWebsite(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/lee/ods/QzWebsite.log").filter(item => {
      val obj = ParseJsonData.getJsonObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzMajor(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/lee/ods/QzMajor.log").filter(item => {
      val obj = ParseJsonData.getJsonObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzBusiness(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/lee/ods/QzBusiness.log").filter(item => {
      val obj = ParseJsonData.getJsonObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzPaperView(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/lee/ods/QzPaperView.log").filter(item => {
      val obj = ParseJsonData.getJsonObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzCenterPaper(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/lee/ods/QzCenterPaper.log").filter(item => {
      val obj = ParseJsonData.getJsonObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzPaper(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/lee/ods/QzPaper.log").filter(item => {
      val obj = ParseJsonData.getJsonObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzCenter(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/lee/ods/QzCenter.log").filter(item => {
      val obj = ParseJsonData.getJsonObject(item)
      obj.isInstanceOf[JSONObject]
  ...
  def etlQzQuestion(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/lee/ods/QzQuestion.log").filter(item => {
      val obj = ParseJsonData.getJsonObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzQuestionType(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/lee/ods/QzQuestionType.log").filter(item => {
      val obj = ParseJsonData.getJsonObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
  ...
  def etlQzMemberPaperQuestion(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/lee/ods/QzMemberPaperQuestion.log").filter(item => {
      val obj = ParseJsonData.getJsonObject(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
